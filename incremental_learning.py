...
t = None
d = {}
path = "incremental_learning.pickle"
...
for (set, seed, lr, batch_size) in trainings:
    ...
    splits = [list(x) for x in np.array_split(cols, 3)]
    splits = [categorise(x, cat_cols, num_cols, bin_cols) for x in splits]
    used_columns = []
    for i, split in enumerate(splits):
        if i == 0:
            model = transtab.build_classifier(categorical_columns=split['cat'], numerical_columns=split['num'],
                                              binary_columns=split['bin'])
            used_columns.extend(to_empty(split['cat'])+to_empty(split['num'])+to_empty(split['bin']))
        else:
            model.update(split)
            used_columns.extend(to_empty(split['cat'])+to_empty(split['num'])+to_empty(split['bin']))
        # Train model on training dataset
        transtab.train(model, (trainset[0][used_columns], trainset[1]), (valset[0][used_columns], valset[1]), lr=lr,
                       batch_size=batch_size, num_epoch=epochs, patience=patience)
    # Compute predictions on test dataset
    y_pred = evaluator.predict(model, testset[0][used_columns])
    # Compute AUROC score
